...
public class SharedCollectionRestControllerUT extends BaseControllerUT {
	...
	public void itemAvailabilityStatus() throws Exception {
		ItemAvailabityStatusRequest itemAvailabityStatus = ItemAvailabityStatusRequest.builder()
				.barcode("32101056185125").build();
		Mockito.when(mockRestTemplate.postForObject(getScsbSolrClientUrl() + "/sharedCollection/itemAvailabilityStatus",
				itemAvailabityStatus, String.class)).thenReturn("Available");
	...
	public void testBibAvailableStatus() {
		...
		Mockito.when(sharedCollectionRestController.getRestTemplate()).thenReturn(mockRestTemplate);
		Mockito.when(sharedCollectionRestController.getScsbSolrClientUrl()).thenReturn(scsbSolrClientUrl);
		Mockito.when(sharedCollectionRestController.bibAvailabilityStatus(bibItemAvailabityStatusRequest))
				.thenCallRealMethod();
		ResponseEntity<?> responseEntity = sharedCollectionRestController
				.bibAvailabilityStatus(bibItemAvailabityStatusRequest);
	...
	public void deAccession() throws Exception {
		...
		DeAccessionItem deAccessionItem = DeAccessionItem.builder().itemBarcode(itemBarcode).deliveryLocation("PB")
				.build();

		DeAccessionRequest deAccessionRequest = DeAccessionRequest.builder()
				.deAccessionItems(Arrays.asList(deAccessionItem)).username("Test").build();
		Map<String, String> resultMap = new HashMap<>();
		resultMap.put(itemBarcode, ReCAPConstants.SUCCESS);
		Mockito.when(mockRestTemplate.postForObject(getScsbCircUrl() + "/sharedCollection/deAccession",
				deAccessionRequest, Map.class)).thenReturn(resultMap);
		Mockito.when(sharedCollectionRestController.getRestTemplate()).thenReturn(mockRestTemplate);
	...
	public void accessionBatch() throws Exception {
		...
		AccessionRequest accessionRequest = AccessionRequest.builder().customerCode("PB").itemBarcode("32101095533293")
				.build();
		accessionRequestList.add(accessionRequest);
		Mockito.when(mockRestTemplate.postForObject(getScsbSolrClientUrl() + "sharedCollection/accessionBatch",
				accessionRequestList, String.class)).thenReturn(ReCAPConstants.SUCCESS);
		Mockito.when(sharedCollectionRestController.getRestTemplate()).thenReturn(mockRestTemplate);
		Mockito.when(sharedCollectionRestController.getScsbSolrClientUrl()).thenReturn(scsbSolrClientUrl);
	...
	public void accession() throws Exception {
		...
		Mockito.when(mockRestTemplate.postForObject(getScsbSolrClientUrl() + "sharedCollection/accession",
				accessionRequestList, List.class)).thenReturn(linkedHashMapList);
		Mockito.when(sharedCollectionRestController.getRestTemplate()).thenReturn(mockRestTemplate);
		Mockito.when(sharedCollectionRestController.getScsbSolrClientUrl()).thenReturn(scsbSolrClientUrl);
	...
	public void submitCollection() throws Exception {
		mockRestTemplate.getMessageConverters().add(0, new StringHttpMessageConverter(Charset.forName("UTF-8")));
		List<LinkedHashMap<String, String>> linkedHashMapList = new ArrayList<>();
		LinkedHashMap<String, String> linkedHashMap = new LinkedHashMap<String, String>();
		...
		Mockito.when(sharedCollectionRestController.getRestTemplate()).thenReturn(mockRestTemplate);
		Mockito.when(sharedCollectionRestController.getScsbCircUrl()).thenReturn(scsbCircUrl);
		Mockito.when(sharedCollectionRestController.getLinkedMultiValueMap())
				.thenReturn((LinkedMultiValueMap<String, Object>) requestParameter);
		Mockito.when(mockRestTemplate.postForObject(getScsbCircUrl() + "sharedCollection/submitCollection",
				requestParameter, List.class)).thenReturn(linkedHashMapList);
